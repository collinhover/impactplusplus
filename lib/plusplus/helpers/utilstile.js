ig.module(
    ...
    .defines(function () {
                ...
                for (iy = 0; iy < height; iy++) {
                    ...
                    for (ix = 0; ix < width; ix++) {
                        ...
                        if (shape.vertices.length > 0) {
                            ...
                            if (HASH_CLIMBABLE[ tile.id ] && !options.ignoreClimbables) {

                                climbables.push(tile);

                            }
                            else if (HASH_ONE_WAY[ tile.id ] && !options.ignoreOneWays) {
         ...
         * <span class="alert alert-info"><strong>Tip:</strong> when converting tiles to shapes, is usually better to call {@link ig.utilstile.shapesFromCollisionMap}.</span>
         * @param {String|Number} tileId tile id
         * @param {Object} [tileDefs=ig.CollisionMap.defaultTileDef] map of tile ids to their definitions
         * @returns {Object} object with array of 0 to 5 vertices and array of 0 to 5 segments
         **/
        ig.utilstile.shapeFromTile = function (tileId, tileDefs) {

            var i, il;
            var vertices = ig.utilstile.verticesFromTile(tileId, tileDefs);
            var segments;
         ...
         **/
        ig.utilstile.verticesFromTile = function (tileId, tileDefs) {

            if ( tileDefs === 'undefined' ) {

                tileDefs = _collisionMapTileDef;
            ...
            if (ig.utilstile.defaultTileVerticesDef[ tileId ]) {
                ...
                if ( parseInt( tileId ) === 1 || HASH_CLIMBABLE[ tileId ]) {
