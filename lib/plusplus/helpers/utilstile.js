/**
 * Static utilities for tiles and collision maps.
...
_utt.defaultTileVerticesDef = {};
_utt.defaultTileSegmentsDef = {};
...
**/
_utt.isTileOneWay = function ( tileId ) {
...
**/
_utt.isTileClimbable = function ( tileId ) {
...
* @param {String} tile id.
* @returns {Boolean} true if tile is climbable and one way
**/
_utt.isTileClimbableOneWay = function ( tileId ) {
	
	return tileId === TILE_CLIMBABLE;
...
**/
_utt.shapesFromCollisionMap = function( map, options ) {
    ...
    if ( map instanceof ig.CollisionMap ) {
        ...
        for( iy = 0; iy < height; iy++ ) {
            ...
            for( ix = 0; ix < width; ix++ ) {
				...
				shape = _utt.shapeFromTile( map, ix, iy );
				...
				if ( shape.vertices.length > 0 ) {
					...
					if ( _utt.isTileClimbable( tile.id ) && !options.ignoreClimbables ) {
						
						climbables.push( tile );
						
					}
					else if ( _utt.isTileOneWay( tile.id ) && !options.ignoreOneWays ) {
		...
		shapes.edges = shapes.edges.concat( _utt.shapedTilesToShapes( solids, data, {
		...
		shapes.climbables = shapes.climbables.concat( _utt.shapedTilesToShapes( climbables, data, {
			rectangles: !options.contourClimbables,
		...
		shapes.oneWays = shapes.oneWays.concat( _utt.shapedTilesToShapes( oneWays, data, {
			rectangles: !options.contourSolids,
			groupByTileId: !options.ungrouped
		} ) );
...
**/
_utt.shapedTilesToShapes = function ( tiles, data, options ) {
	...
	if ( options.groupByTileId ) {
		...
		for ( i = 0, il = ids.length; i < il; i++ ) {
			...
			shapes = shapes.concat( _utt.shapedTilesToShapes( group, data, options ) );
		...
		if ( options.rectangles ) {
			...
			while ( tilePool.length > 0 ) {
				...
				line = _utt.findShapedTileLine( tilePool );
				...
				while ( stepped ) {
					...
					if ( tileFrom ) {
						...
						if ( index !== -1 ) {
							...
							line = _utt.findShapedTileLine( tilePool, false, index, length );
			...
			for ( j = 0, jl = rectangles.length; j < jl; j++ ) {
				...
				for ( i = 0, il = rectangle.length; i < il; i++ ) {
					...
					vertices = vertices.concat( _utt.getNonDuplicateSegmentVertices( rectangle[ i ], data, rectangle ) );
					
				}
				
				// vertices to contours
				
				contours = contours.concat( _utt.verticesToContours( vertices, options ) );
		...
		// general shapes that may or may not be concave
			...
			for ( i = 0, il = tiles.length; i < il; i++ ) {
				...
				vertices = vertices.concat( _utt.getNonDuplicateSegmentVertices( tiles[ i ], data, tiles ) );
				
			}
			
			// vertices to contours
			
			contours = _utt.verticesToContours( vertices, options );
...
**/
_utt.findShapedTileLine = function ( tiles, horizontal, indexFrom, length ) {
	...
	while( stepped ) {
		...
		var tileNext = horizontal ? _utt.stepShapedTileHorizontally( tiles, tileFrom ) : _utt.stepShapedTileVertically( tiles, tileFrom );
...
**/
_utt.stepShapedTileHorizontally = function ( tiles, tileFrom ) {
...
* @returns {Object} next tile, or current tile if next not found.
**/
_utt.stepShapedTileVertically = function ( tiles, tileFrom ) {
...
**/
_utt.verticesToContours = function ( vertices, options ) {
		...
		while ( contourPool.length > -1 ) {
			...
			if ( contour ) {
				...
				for( i = contours.length - 1; i > -1; i--) {
					...
					if ( contour !== containingContour && _uti.AABBContainedByAABB( contour.minX, contour.minY, contour.maxX, contour.maxY, containingContour.minX, containingContour.minY, containingContour.maxX, containingContour.maxY ) ) {
...
**/
_utt.shapeFromTile = function ( map, ix, iy ) {
	
    var i, il;
	var tileId = map.data[ iy ][ ix ];
	var vertices = _utt.verticesFromTile( map, ix, iy );
	var segments;
	...
	if ( vertices ) {
		...
		if ( _utt.defaultTileSegmentsDef[ tileId ] ) {
			
			segments = _utt.defaultTileSegmentsDef[ tileId ];
			
		}
		else {
			
			_utt.defaultTileSegmentsDef[ tileId ] = segments = [];
...
**/
_utt.verticesFromTile = function ( map, ix, iy ) {
	...
	if ( _utt.defaultTileVerticesDef[ tileId ] ) {
		
		vertices = _utt.defaultTileVerticesDef[ tileId ];
		...
		if ( tileId === 1 || _utt.isTileClimbable( tileId ) ) {
		...
		// solid sloped
			...
			_utt.defaultTileVerticesDef[ tileId ] = vertices;
...
**/
_utt.getNonDuplicateSegmentVertices = function ( tile, tileData, tilesAllowed ) {
	...
	for ( i = 0, il = segments.length; i < il; i++ ) {
		...
		if ( ( normal.x === 0 && normal.y !== 0 ) || ( normal.x !== 0 && normal.y === 0 ) ) {
			...
			var touchingTiles = _utt.getTouchingTilesByDirection( tile, normal, tileData, tilesAllowed );
			...
			for ( j = 0, jl = touchingTiles.length; j < jl; j++ ) {
				...
				if ( touchingTile.shape.vertices.length > 0 ) {
					...
					overlap = _utt.getSegmentOverlapWithTile( va, vb, normal, touchingTile );
 ...
 **/
_utt.getTouchingTilesByDirection = function ( tile, direction, tileData, tilesAllowed ) {
 ...
 **/
_utt.getSegmentOverlapWithTile = function ( vaA, vbA, normalCompare, tile ) {
