ig.module(
    ...
    .defines(function() {
        ...
        ig.utilsintersection.AABBContains = function(aminX, aminY, amaxX, amaxY, bminX, bminY, bmaxX, bmaxY) {
            ...
            if (aminX >= bminX && amaxX <= bmaxX && aminY >= bminY && amaxY <= bmaxY) {
                return true;
            }
         ...
         * @param {Number} minX left position.
         * @param {Number} minY top position.
         * @param {Number} maxX right position.
         * @param {Number} maxY bottom position.
         ...
         */
        ig.utilsintersection.entitiesInAABB = function(minX, minY, maxX, maxY, targetable, layerName, unsorted) {
            ...
            if (!layerName) {
                ...
                for (i = 0, il = ig.game.layers.length; i < il; i++) {
                    ...
                    if (layer.name && layer.numEntities && (!targetable || layer.itemsTargetable.length)) {
                        ...
                        intersected = intersected.concat(ig.utilsintersection.entitiesInAABB(minX, minY, maxX, maxY, targetable, layer.name, true));
            ...
            if (intersected.length > 1 && !unsorted) {
                ...
                ig.utilsintersection.sortByDistance(minX + (maxX - minX) * 0.5, minY + (maxY - minY) * 0.5, intersected);
        ...
        /**
         * Sort a list of entities by distance to a point in world space coordinates.
         ...
         **/
        ig.utilsintersection.sortByDistance = function(x, y, entities) {

            var pos = _utv2.set(ig.utilsintersection._utilVec2Sort, x, y);

            entities.sort(function(a, b) {

                var aDistance = a.distanceSquaredEdgeTo(pos);
                var bDistance = b.distanceSquaredEdgeTo(pos);
        ...
        ig.utilsintersection._utilVec2Sort = {
            x: 0.1,
            y: 0.1
        };
