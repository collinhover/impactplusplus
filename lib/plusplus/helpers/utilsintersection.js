    ...
    .defines(function () {
        ...
        ig.utilsintersection.entitiesInAABB = function (left, top, right, bottom, targetable, layerName, unsorted) {
            ...
            if (intersected.length > 1 && !unsorted) {
                ...
                var xCenter = left + ( right - left ) * 0.5;
                var yCenter = top + ( bottom - top ) * 0.5;

                intersected.sort(function sortByDistance(a, b) {

                    var aToCenterX = xCenter - a.getCenterX();
                    var aToCenterY = yCenter - a.getCenterY();
                    var aDistance = aToCenterX * aToCenterX + aToCenterY * aToCenterY;

                    var bToCenterX = xCenter - b.getCenterX();
                    var bToCenterY = yCenter - b.getCenterY();
