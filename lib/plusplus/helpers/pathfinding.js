ig.module(
    ...
    .defines(function() {
         ...
         * @param {Number} [gridOffsetX=0] direction to offset by 1 node on x.
         * @param {Number} [gridOffsetY=0] direction to offset by 1 node on y.
         * @returns {ig.pathfinding.Node} node at point.
         **/
        ig.pathfinding.getWorldNodeAt = function(x, y, gridOffsetX, gridOffsetY ){

            var gridX = Math.floor(x * _mapTilesizeReciprocal) + ( gridOffsetX > 0 ? 1 : ( gridOffsetX < 0 ? -1 : 0 ) );
            var gridY = Math.floor(y * _mapTilesizeReciprocal) + ( gridOffsetY > 0 ? 1 : ( gridOffsetY < 0 ? -1 : 0 ) );
