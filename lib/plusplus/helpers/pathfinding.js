ig.module(
    ...
    .defines(function() {
            ...
            init: function ( x, y, gridX, gridY, walkable, tile ) {
                ...
                if ( this.sloped ) {
                    ...
                    if ( segmentsDef ) {
                        ...
                        for ( var i = 0, il = segmentsDef.length; i < il; i++ ) {
        ...
        ig.pathfinding.rebuildGrid = function () {
            ...
            for ( y = 0, yl = _grid.length; y < yl; y++ ) {
                ...
                for ( x = 0, xl = row.length; x < xl; x++ ) {
                    ...
                    node.slopesAlongMap.bottom = node.walkable && nm.bottom && !nm.bottom.walkable;
        ...
        ig.pathfinding.getPositionPathfindingEntities = function ( entityPathing, entityTarget, positions ) {
            ...
            var pZeroGrav = entityPathing.gravityFactor === 0 || ig.game.gravity === 0;
            ...
            var tZeroGrav = entityTarget.gravityFactor === 0 || ig.game.gravity === 0;
            ...
            if ( pMinY > tMaxY ) {
                ...
                positions.from.y = pZeroGrav ? pMinY + tilesizeQuarter : pMaxY - tilesizeQuarter;
                ...
                positions.target.y = tZeroGrav ? tMinY + tilesizeQuarter : tMaxY - tilesizeQuarter;

            }
            else {

                positions.from.y = pZeroGrav ? pMinY + pHeight * 0.5 : pMaxY - tilesizeQuarter;
                positions.target.y =  tZeroGrav ? tMinY + tHeight * 0.5 : tMaxY - tilesizeQuarter;
        ...
        ig.pathfinding.getPositionPathfindingPoint = function ( entityPathing, targetX, targetY, position ) {
            ...
            var pMinX = entityPathing.pos.x;
            var pMaxX = pMinX + entityPathing.size.x;
            var pMinY = entityPathing.pos.y;
            var pMaxY = pMinY + entityPathing.size.y;

            var tilesizeQuarter = _tilesize * 0.25;

            if ( pMinX + tilesizeQuarter > targetX ) {

                position.x = pMinX + tilesizeQuarter;

            }
            else if ( pMaxX - tilesizeQuarter < targetX ) {

                position.x = pMaxX - tilesizeQuarter;

            }
            else {

                position.x = pMinX + entityPathing.size.x * 0.5;

            }

            if ( !( entityPathing.gravityFactor === 0 || ig.game.gravity === 0 ) ) {

                position.y = pMaxY - tilesizeQuarter;

            }
            else if ( pMinY + tilesizeQuarter > targetY ) {

                position.y = pMinY + tilesizeQuarter;

            }
            else if ( pMaxY - tilesizeQuarter < targetY ) {
                ...
                position.x = pMinY + entityPathing.size.y * 0.5;
        ...
        ig.pathfinding.getBoundsPathfollowing = function ( x, y, width, height, target, zeroGrav, bounds ) {
            ...
            if ( target.corner ) {
                ...
                tileWidth = Math.min( _tilesize * 0.5, width * 0.5 );
                tileHieight = Math.min( _tilesize * 0.5, height * 0.5 );
                ...
                if ( target.cornerMap.bottomleft ) {
                    ...
                    bounds.minY = bounds.maxY - tileHieight;
                    ...
                    bounds.maxX = bounds.minX + tileWidth;
                    bounds.minY = y;
                    bounds.maxY = bounds.minY + tileHieight;
            ...
            // modify bounds away from node based on node size
                ...
                tileWidth = Math.min( _tilesize, width );
                tileHieight = Math.min( _tilesize, height );
                ...
                if ( zeroGrav ) {
                    ...
                    bounds.minY = y + height * 0.5 - tileHieight * 0.5;
                    bounds.maxY = bounds.minY + tileHieight;
        ...
        ig.pathfinding.getEntityOnSmoothSlope = function ( entity, dirX, dirY ) {
            ...
            if ( dirX < 0 ) {
                ...
                node = ig.pathfinding.getWorldNodeAt( entity.pos.x + tilesizeQuarter, entity.pos.y + entity.size.y - tilesizeQuarter );

            }
            else {

                node = ig.pathfinding.getWorldNodeAt( entity.pos.x + entity.size.x - tilesizeQuarter, entity.pos.y + entity.size.y - tilesizeQuarter );
