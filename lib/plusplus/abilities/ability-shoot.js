        ...
        ig.AbilityShoot = ig.Ability.extend(/**@lends ig.AbilityShoot.prototype */{
            ...
            activateComplete: function (settings) {
                ...
                if (settings) {
                    ...
                    length = Math.sqrt(offsetX * offsetX + offsetY * offsetY) || 1;
                    ...
                    x += offsetX;
                    y += offsetY;
                    var normalX = offsetX / length;
                    var normalY = offsetY / length;

                }

                x = x.limit( minX + width * this.shootLocationMinPctX, minX + width * this.shootLocationMaxPctX );
                y = y.limit( minY + height * this.shootLocationMinPctY, minY + height * this.shootLocationMaxPctY );

                if ( ig.game.isInsideLevel( x, y ) ) {
