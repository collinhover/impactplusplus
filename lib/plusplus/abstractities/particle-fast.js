ig.module(
    ...
    .defines(function () {
        ...
        ig.ParticleFast = ig.Particle.extend(/**@lends ig.ParticleFast.prototype */{
            ...
            /**
             * Fast particles have an abbreviated init where some methods, such as initTypes, are skipped.
             * @override
             */
             ...
             * @override
             */
            tween: function () { throw new Error("Attempting to TWEEN fast particle but fast particle does not tween!"); },

            /**
             * Fast particles don't collide with other entities.
             * @override
             **/
            collideWith: function () { throw new Error("Attempting to COLLIDE fast particle but fast particle does not collide!"); },

            /**
             * Fast particles don't collide with collision map.
             * @override
             **/
            handleMovementTrace: function () { throw new Error("Attempting to TRACE COLLISION MAP fast particle but fast particle ignore collision map!"); },
            ...
            cleanup: function () {
                ...
                if ( !ig.game.hasLevel ) {
                    ...
                    this.currentAnim.onCompleted.removeAll();
                    this.currentAnim.onCompleted.forget();
            ...
            getIsVisible: function () {
                ...
                if (this.alpha <= 0) return false;
                    ...
                    if (minX < 0 || minY < 0
                        || minX + this.sizeDraw.x > ig.system.width
                        || minY + this.sizeDraw.y > ig.system.height) return false;
             ...
             * @override
             */
            recordChanges: function () { throw new Error("Attempting to RECORD CHANGES in fast particle but fast particle is always changed!"); },

            /**
             * Fast particles never records last.
             * @override
             */
            recordLast: function () {throw new Error("Attempting to RECORD LAST of fast particle but fast particle doesn't record last!");},

            /**
             * Fast particles never records last.
             * @override
             */
            updateBounds: function () {throw new Error("Attempting to UPDATE BOUNDS of fast particle but fast particle does not update bounds!");},
                    ...
                    if ( image.loaded ) {
                        ...
                        if (this.alpha !== 1) {
                            ...
                            ig.system.context.drawImage(
                                image.data,
                                ( Math.floor(anim.tile * tileWidth) % image.width ) * scale,
                                ( Math.floor(anim.tile * tileWidth / image.width) * tileHeight ) * scale,
                                tileWidthScaled,
                                tileHeightScaled,
