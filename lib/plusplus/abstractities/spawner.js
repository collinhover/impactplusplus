ig.module(
    ...
    .defines(function () {
        ...
        ig.Spawner = ig.EntityTrigger.extend(/**@lends ig.Spawner.prototype */{
            ...
            /**
             * Whether to spawn at the bottom of bounds of {@link ig.Spawner#spawnAt}.
             * @type Boolean
             * @default
             */
            spawnPreferBottom: false,
                ...
                if (!this.spawning && this.spawnCount < this.spawnCountMax && ( this.spawningEntity || this.pool.length > 0 )) {
                    ...
                    // over duration
                        ...
                        if (this.spawnDelay < 0 && this.duration !== -1) {
                            ...
                            this.spawnDelay = this.duration / this.spawnCountMax;
            ...
            spawnNext: function (entity) {
                ...
                if (this.spawnAtRandomPosition) {
                    ...
                    x = Math.random().map(0, 1, this.spawnAt.bounds.minX, this.spawnAt.bounds.maxX);
                    y = Math.random().map(0, 1, this.spawnAt.bounds.minY, this.spawnAt.bounds.maxY);

                }
                else {

                    x = this.spawnAt.getCenterX();
                    y = this.spawnAt.getCenterY();
            ...
            spawned: function (entity) {
                ...
                if (this.spawnPreferBottom) {

                    entity.pos.y = this.spawnAt.bounds.maxY - entity.bounds.height;

                }
                // check location of spawn relative to this bounds and entity's bounds
                else {

                    if (this.spawnAt.bounds.width > entity.bounds.width) {

                        if (entity.bounds.maxX > this.spawnAt.bounds.maxX) {

                            entity.pos.x = this.spawnAt.bounds.maxX - entity.bounds.width;

                        }
                        else if (entity.bounds.minX < this.spawnAt.bounds.minX) {

                            entity.pos.x = this.spawnAt.bounds.minX;

                        }

                    }

                    if (this.spawnAt.bounds.height > entity.bounds.height) {

                        if (entity.bounds.maxY > this.spawnAt.bounds.maxY) {

                            entity.pos.y = this.spawnAt.bounds.maxY - entity.bounds.height;

                        }
                        else if (entity.bounds.minY < this.spawnAt.bounds.minY) {

                            entity.pos.y = this.spawnAt.bounds.minY;
            ...
            update: function () {
                ...
                if (this.spawning && ( this.duration === 0 || this.spawnTimer.delta() >= 0 )) {

                    this.spawnTimer.set(this.spawnDelay);
                    var entitySpawned = this.spawnNext();

                    if (this.durationTimer.delta() >= 0 || !entitySpawned) {
