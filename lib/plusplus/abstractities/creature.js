...
.defines(function(){ 
    ...
    ig.Creature = ig.Character.extend( {
         ...
         * @default
         */
        typePrey: ig.EntityExtended.TYPE.NONE,
         ...
         * @default
         */
        typePredator: ig.EntityExtended.TYPE.NONE,
        ...
        /**
         * Name of tether entity. If not present will try to use spawner as tether.
         * @type String
         */
        tetherName: '',
		...
		resetCore: function (x, y, settings) {
			...
			if ( settings && !ig.global.wm ) {
				...
				if ( settings.typePrey ) {
					
					_ut.addType(ig.EntityExtended, this, 'typePrey', settings.typePrey);
					delete settings.typePrey;
					
				}

				if ( settings.groupPrey ) {

					_ut.addType(ig.EntityExtended, this, 'groupPrey', settings.groupPrey, "GROUP");
					delete settings.groupPrey;

				}

                if ( settings.typePredator ) {

                    _ut.addType(ig.EntityExtended, this, 'typePredator', settings.typePredator);
                    delete settings.typePredator;

                }
				
				if ( settings.groupPredator ) {
					
					_ut.addType(ig.EntityExtended, this, 'groupPredator', settings.groupPredator, "GROUP");
					delete settings.groupPredator;
            ...
            if ( needsPrey || needsPredator ) {
                ...
                if ( needsPrey ) {
                    ...
                    if ( this.namePrey ) {
                        ...
                        var prey = ig.game.getEntityByName( this.namePrey );
                        ...
                        if ( prey
                            ...
                            this.entityPrey = prey;

                        }

                    }
                    else if ( this.classPrey ) {

                        this.entityPrey = ig.game.getEntitiesByClass( this.classPrey, {
                            x: cX,
                            y: cY,
                    ...
                    else if ( this.typePrey ) {

                        this.entityPrey = ig.game.getEntitiesByType( this.typePrey, {
                            x: cX,
                            y: cY,
                            layerName: this.layerName,
                            byDistance: true,
                    ...
                    else if ( this.groupPrey ) {

                        this.entityPrey = ig.game.getEntitiesByGroup( this.groupPrey, {
                            x: cX,
                            y: cY,
                            layerName: this.layerName,
                            byDistance: true,
                ...
                if ( needsPredator ) {
                    ...
                    if ( this.namePredator ) {

                        var predator = ig.game.getEntityByName( this.namePredator );
                        ...
                        if ( predator
                            ...
                            this.entityPredator = predator;

                        }

                    }
                    else if ( this.classPredator ) {

                        this.entityPredator = ig.game.getEntitiesByClass( this.classPredator, {
                            x: cX,
                            y: cY,
                            layerName: this.layerName,
                            byDistance: true,
                    ...
                    else if ( this.typePredator ) {

                        this.entityPredator = ig.game.getEntitiesByType( this.typePredator, {
                            x: cX,
                            y: cY,
                            layerName: this.layerName,
                            byDistance: true,
                    ...
                    else if ( this.groupPredator ) {

                        this.entityPredator = ig.game.getEntitiesByGroup( this.groupPredator, {
                            x: cX,
                            y: cY,
                            layerName: this.layerName,
                            byDistance: true,
            ...
            if ( this.tetherDistance > 0 ) {
                ...
                if ( tether ) {
                    ...
                    if ( this.tethering ) {
                        ...
                        if ( ( !this.canBreakTether || ( !this.entityPrey && !this.entityPredator ) )
                            && this.distanceSquaredEdgeTo( this.entityTether ) > this.tetherDistance * this.tetherDistance ) {

                            this.fleeing = false;
                            this.entityPrey = this.entityPredator = null;
                            this.moveToStop();
