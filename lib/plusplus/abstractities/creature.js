        ...
        */
        findPredatorPrey: function ( force ) {
            ...
            if ( needsPrey || needsPredator ) {
                ...
                if ( this.reactionDistance < Number.MAX_VALUE ) {

                    minX = cX - this.reactionDistance;
                    minY = cY - this.reactionDistance;
                    maxX = cX + this.reactionDistance;
                    maxY = cY + this.reactionDistance;
                ...
                if ( needsPrey ) {
                    ...
                    if ( this.preyName ) {
                        ...
                        var preySearchSettings = this._preySearchSettings;
                        preySearchSettings.minX = minX;
                        preySearchSettings.minY = minY;
                        preySearchSettings.maxX = maxX;
                        preySearchSettings.maxY = maxY;

                        if ( this.preyClass ) {

                            this.prey = ig.game.getEntitiesByClass( this.preyClass, preySearchSettings )[ 0 ];

                        }
                        else if ( this.preyType ) {

                            this.prey = ig.game.getEntitiesByType( this.preyType, preySearchSettings )[ 0 ];

                        }
                        else if ( this.preyGroup ) {

                            this.prey = ig.game.getCharactersByGroup( this.preyGroup, preySearchSettings )[0];
                ...
                if ( needsPredator ) {
                    ...
                    if ( this.predatorName ) {
                        ...
                        var predatorSearchSettings = this._preySearchSettings;
                        predatorSearchSettings.minX = minX;
                        predatorSearchSettings.minY = minY;
                        predatorSearchSettings.maxX = maxX;
                        predatorSearchSettings.maxY = maxY;

                        if ( this.predatorClass ) {

                            this.predator = ig.game.getEntitiesByClass( this.predatorClass, predatorSearchSettings )[ 0 ];

                        }
                        else if ( this.predatorType ) {

                            this.predator = ig.game.getEntitiesByType( this.predatorType, predatorSearchSettings )[ 0 ];

                        }
                        else if ( this.predatorGroup ) {

                            this.predator = ig.game.getCharactersByGroup( this.predatorGroup, predatorSearchSettings )[0];
        ...
        cleanup: function () {
            ...
            this.removePrey();
            this.removePredator();
            this.tethering = this.wandering = false;
        ...
        updateWander: function () {
            ...
            if ( ( this.canWanderX || this.canWanderY ) && !this.tethering && !this.prey && !this.predator ) {
                ...
                // has direction
                    ...
                    if ( this.wanderDirection.x !== 0 ) {
						...
						if ( this.wanderDirection.x > 0 ) {
							...
							this.wanderPos.x = this.pos.x + this.size.x + 1;
							
						}
						else {
							
							this.wanderPos.x = this.pos.x - 1;
					...
					if ( this.wanderDirection.y !== 0 ) {
						...
						if ( this.wanderDirection.y > 0 ) {
							...
							this.wanderPos.y = this.pos.y + this.size.y + 1;
							
						}
						else {
							
							this.wanderPos.y = this.pos.y - 1;
