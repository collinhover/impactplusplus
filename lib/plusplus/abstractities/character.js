            ...
            findMoveToPath: function ( settings ) {
                ...
                if ( this.pathingTimer.delta() >= 0 && ( this.path.length === 0 || this.needsNewPath ) ) {
                    ...
                    if ( simple ) {
                        ...
                        var node = unsafe || onSlope ? _pf.getWorldNodeAt( from.x, from.y, dX, dY ) : _pf.getWorldWalkableNodeAt( from.x, from.y, dX, dY, avoidEntities, this );
						...
						if ( !this.hasGravity ) {
							...
							if ( !this.jumping ) {
								...
								if ( minY <= nodeY && maxY >= nodeY ) {
                                    ...
                                    var needsUp = withinX || !_pf.getEntityOnSmoothSlope( this, nodeX - cX, -1 );
									this.moveToUnsafe = !unsafe && node.ungrounded && needsUp;
