            ...
            findMoveToPath: function ( settings ) {
                ...
                if ( this.pathingTimer.delta() >= 0 && ( this.path.length === 0 || this.needsNewPath ) ) {
                    ...
                    if ( simple ) {
                        ...
                        var hasGravity = !( this.gravityFactor === 0 || ig.game.gravity === 0 );
                        ...
                        if ( this.movingTo instanceof ig.EntityExtended ) {
                            ...
                            // move to target's feet when affected by gravity
                            dY = ( this.movingTo.pos.y + this.movingTo.size.y * ( hasGravity ? 0.95 : 0.5 ) ) - y;
                        ...
                        // when gravity affects entity, only allow vertical movement when 2x greater than horizontal distance

                        if ( dX !== 0 && dY !== 0 && hasGravity ) {
                        ...
                        // and make sure distance to move is more than half character size
                        ...
                        if ( dX !== 0 ) {
                            ...
                            if ( dX * dX < this.size.x * this.size.x * 0.25 ) {
                                ...
                                eX = this.pos.x + ( this.size.x * 0.5 > _pf.tilesize * 0.5 ? this.size.x - _pf.tilesize * 0.5 : this.size.x * 0.5 );
                                dX = 1;

                            }
                            else {

                                eX = this.pos.x + ( this.size.x * 0.5 > _pf.tilesize * 0.5 ? _pf.tilesize * 0.5 : this.size.x * 0.5 );
                                dX = -1;
                        ...
                        if ( dY !== 0 ) {
                            ...
                            if ( dY * dY < this.size.y * this.size.y * 0.25 ) {
                                ...
                                if ( dY > 0 ) {
                                    ...
                                    eY = this.pos.y + ( this.size.y * 0.5 > _pf.tilesize * 0.5 ? this.size.y - _pf.tilesize * 0.5 : this.size.y * 0.5 );
                                    dY = 1;

                                }
                                else {

                                    eY = this.pos.y + ( this.size.y * 0.5 > _pf.tilesize * 0.5 ? _pf.tilesize * 0.5 : this.size.y * 0.5 );
                                    dY = -1;
                        ...
                        var node = unsafe ? _pf.getWorldNodeAt( eX, eY, dX, dY ) : _pf.getWorldWalkableNodeAt( eX, eY, dX, dY, avoidEntities, this );
						...
						if ( this.gravityFactor === 0 || ig.game.gravity === 0 ) {
							...
							if ( !this.jumping ) {
								...
								if ( minY <= nodeY && maxY >= nodeY ) {
									...
									this.moveToUnsafe = !unsafe && node.ungrounded && !node.slopedNeighbor;
