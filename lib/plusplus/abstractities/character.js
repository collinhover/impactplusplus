ig.module(
    ...
    .defines(function () {
        ...
        ig.Character = ig.EntityExtended.extend(/**@lends ig.Character.prototype */{
             ...
             * @default
             */
            canJump: _c.CHARACTER.CAN_JUMP && !_c.TOP_DOWN.ENABLED,
             ...
             * @default
             */
            canClimb: _c.CHARACTER.CAN_CLIMB && !_c.TOP_DOWN.ENABLED,
            ...
            /**
             * Whether character pathfinding is simple and based only on direction.
             * <span class="alert alert-info"><strong>Tip:</strong> simple pathfinding is much faster but far less accurate than full pathfinding!</span>
             * @type Boolean
             * @see ig.CONFIG.CHARACTER.PATHFIND_SIMPLE
             */
            pathfindSimple: _c.CHARACTER.PATHFIND_SIMPLE,
             ...
             * // if a character cannot jump
             * character.canJump = false;
             * // it will not move when the path goes into the air
             * // it will not move when the path goes off an edge
             * // if a character cannot climb
             * character.canClimb = false;
             * // it will not move when the path goes into a climbing area
             * // unless the path is going within a grounded area
             */
            ...
            /**
             * Moves character away from entity instead of towards.
             * <br>- this method hooks into the moveToEntity method and adds some extra properties
             * @param {ig.EntityExtended} entity entity to move away from
             * @param {Object} [settings] settings object
             * @returns {Boolean} whether a new move from has been started
             * @see ig.Character#moveToEntity
             **/
            moveFromEntity: function (entity, settings) {

                var movingTo = this.moveToEntity( entity, settings );
            ...
            findMoveToPath: function () {
                ...
                if ( this.pathingTimer.delta() >= 0 && ( this.path.length === 0 || this._dirtyPathEntities ) ) {
                    ...
                    if ( this.pathfindSimple ) {

                        var cx = this.bounds.minX + this.bounds.width * 0.5;
                        var cy = this.bounds.minY + this.bounds.height * 0.5;
                        var dx = ( this.movingToEntity.bounds.minX + this.movingToEntity.bounds.width * 0.5 ) - cx;
                        var dy = ( this.movingToEntity.bounds.minY + this.movingToEntity.bounds.height * 0.5 ) - cy;
                        ...
                        var node = _pf.getWorldWalkableNodeAt( cx, cy, dx, dy );
                    ...
                    // complex pathfinding
                        ...
                        if ( this.movingFrom ) {
                            ...
                            this.path = _pf.getPathAwayFromEntity( this, this.movingToEntity, this.pathfindAroundEntities, this.pathfindingDistance );
                    ...
                    // follow path
                        ...
                        if ( !this.jumping ) {
                            ...
                            if ( minY <= nodeY && maxY >= nodeY ) {
                                ...
                                unreachable = !this.pathfindUnreachable && !node.slopedNeighbor;
            ...
            moveToUp: function (mod) {
                ...
                this.flip.y = _c.TOP_DOWN.ENABLED && _c.TOP_DOWN.FLIP_Y;
                ...
                if (this.movingToSequence) {
                    ...
                    if (this.movingToSequence.length > 0) {
                        ...
                        if ( this.movingFrom ) {
                            ...
                            this.movingTo = this.movingFrom = false;

                            this.moveFromEntity(this.movingToSequence.shift(), this.movingToSettings);
             ...
             * <vr>If top_down is enabled it will handle x/y movement. If top-down is disabled it:
                ...
                if (this.performance === _c.DYNAMIC) {
                    ...
                    if (_c.TOP_DOWN.ENABLED) {
                        ...
                        if (this.movingX && !this.movingY && this.anims.moveX && this.currentAnim !== this.anims.moveX) {
                            ...
                            if (_c.TOP_DOWN.FLIP_Y && this.anims.moveY && this.currentAnim !== this.anims.moveY) {

                                this.currentAnim = this.anims.moveY;

                            }
                            else if (this.anims.moveUp && this.anims.moveDown) {

                                if (this.vel.y > 0 && this.currentAnim !== this.anims.moveDown) {

                                    this.currentAnim = this.anims.moveDown;

                                }
                                else if(this.vel.y < 0 && this.currentAnim !== this.anims.moveUp) {
