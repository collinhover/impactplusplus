ig.module(
    ...
    .defines(function () {
        ...
        ig.GameExtended = ig.Game.extend(/**@lends ig.GameExtended.prototype */{
            ...
            /**
             * Class of preloader to use when creating (@link ig.GameExtended#preloader).
             * @type ig.Preloader
             * @default
             */
            preloaderClass: ig.Preloader,

            /**
             * Level preloader for asynchronous loading.
             * <br>- created on init.
             * @type ig.Preloader
             */
            preloader: null,
            ...
            init: function () {
                ...
                this.preloader = new (this.preloaderClass)();
            ...
            loadLevel: function (level, playerSpawnerName) {
                ...
                // Delay the levelchange

                if ( _c.PRELOADER.ENABLED && !this.preloader.ready ) {
                    ...
                    this.transitioner.fadeTo(1, {
                        onComplete: function () {
                            ...
                            me.unloadLevelDeferred();
                            me.preloader.delayLevelLoad(level, me._playerSpawnerName);
                            me.preloader.onReady.addOnce(function() {

                                me.loadLevelDeferred(this.delayedLevel, this.delayedSpawnerName)
            ...
            buildLevel: function () {
                ...
                if ( levelData ) {
                    ...
                    var levelChangeEntities = [];

                    for (i = 0, il = levelData.entities.length; i < il; i++) {

                        entData = levelData.entities[ i ];

                        ent = this.spawnEntity(entData.type, entData.x, entData.y, entData.settings);

                        if ( _c.PRELOADER.ENABLED && ent instanceof ig.EntityLevelchange ) {

                            levelChangeEntities.push(ent);

                        }
                    }

                    // preload linked levels

                    if ( _c.PRELOADER.ENABLED && levelChangeEntities.length ) {

                        for (i = 0, il = levelChangeEntities.length; i < il; i++) {

                            var level = levelChangeEntities[i].levelName;
                            var levelName = level.replace(/^(Level)?(\w)(\w*)/, function (m, l, a, b) {
                                return a.toUpperCase() + b;
                            });
                            ...
                            if (!ig.global['Level' + levelName]) {

                                this.preloader.queueLevel(levelName);

                            }

                        }

                        if (this.preloader.getQueueLength()) {

                            this.preloader.loadLevels();
            ...
            createShapes: function () {
                ...
                if ( this.shapesPasses ) {
                    ...
                    for (i = 0, il = this.shapesPasses.length; i < il; i++) {
                        ...
                        for (j = 0, jl = shapes.solids.length; j < jl; j++) {

                            var shape = shapes.solids[ j ];
            ...
            /**
             * Get the player object. This method searches for name, then class, then type.
             * @param {Object} [settings] settings for search.
             * @returns {ig.Player} player if exists in game.
             **/
            getPlayer: function (settings) {

                return this.namedEntities[ "player" ]
                    || this.getEntitiesByClass( "Player", settings || { first: true } )[0]
                    || this.getEntitiesByType( ig.EntityExtended.TYPE.PLAYER, settings || { first: true } )[0];
                ...
                if (matching.length > 1 && !unsorted) {
                    ...
                    if ( byDistance ) {
                        ...
                        if ( from instanceof ig.EntityExtended ) {
                            ...
                            _uti.sortByDistance( from.pos.x + from.width * 0.5, from.pos.y + from.height * 0.5, matching );
