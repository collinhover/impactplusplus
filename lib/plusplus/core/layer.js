    ...
    .defines(function () {
        ...
        ig.Layer = ig.Class.extend(/**@lends ig.Layer.prototype */{
                ...
                if ( this._itemsToAdd.length > 0 ) {
                    ...
                    for ( i = 0, il = this._itemsToAdd.length; i < il; i++ ) {
                        ...
                        if ( item._wasLinkedTo ) {
                            ...
                            ig.game.spawnEntity( item, item.pos.x, item.pos.y, { linkedTo: item._wasLinkedTo } );
                            delete item._wasLinkedTo;

                        }
                        else {

                            ig.game.spawnEntity( item, item.pos.x, item.pos.y, ( item._wasLinkedTo ? { linkedTo: item._wasLinkedTo } : undefined ) );
            ...
            _addEntity: function ( item ) {
                ...
                if (
                    item.type !== ig.Entity.TYPE.NONE
                    || item.checkAgainst !== ig.EntityExtended.TYPE.NONE
                    || item.collides !== ig.EntityExtended.COLLIDES.NEVER
            ...
            update: function () {
                ...
                if (!this.noUpdate && this.items.length > 0) {
                    ...
                    for (i = 0, il = this.items.length; i < il; i++) {
                        ...
                        if (!item.highPerformance && !item.paused && (item.changed || item.checking) ) {
                    ...
                    if ( this.changed && this.itemsChecking.length > 0 ) {
