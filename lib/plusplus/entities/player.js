...
.requires(
	...
	'plusplus.core.character-mobile',
...
ig.global.EntityPlayer = ig.EntityPlayer = ig.CharacterMobile.extend({
	...
	/*
	 * Finds all entities intersecting box.
	 * @param {Number} x position to look at.
	 * @param {Number} y position to look at.
	 * @returns {Array} All entities under touch.
	 */
	intersectEntities: function ( left, top, right, bottom ) {
		
		var entities = this.getLayer().items;
		var intersected = [];
		...
		// sort by z index with highest first
		
		intersected.sort( function ( a, b ) {
	...
	mimicTouching: function () {
		...
		var t = this.touch;
		var intersected = this.intersectEntities( t.worldx, t.worldy, t.worldx, t.worldy );
