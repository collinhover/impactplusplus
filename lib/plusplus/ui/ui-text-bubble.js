        ...
        ig.UITextBubble = ig.global.UITextBubble = ig.UIOverlay.extend(/**@lends ig.UITextBubble.prototype */{
            ...
            resetExtras: function () {
                ...
                if ( !this.sizeAsPct ) {
                    ...
                    this.resetState.sizeBase = this.resetState.sizeBase || _utv2.vector();
                    _utv2.copy(this.resetState.sizeBase, this.size);
            ...
            refresh: function (force) {
                ...
                if ( !this.sizeAsPct ) {
                    ...
                    _utv2.copy(this.size, this.resetState.sizeBase);
            ...
            resize: function ( force ) {
                ...
                if ( this.message ) {
                    ...
                    var sizeX = this.getTotalSizeX() - this.padding.x * 2;
                    var sizeY = this.getTotalSizeY() - this.padding.y * 2 - this.triangleLength;
                    var scaleMod = this.message.scale / this.scale;
                    var messageSizeX = this.message.boundsDraw.width * scaleMod;

                    // resize text to fit inside bubble

                    if ( messageSizeX > sizeX ) {

                        this.message.maxWidth = sizeX * scaleMod;

                        this.message.refresh( true );
                        messageSizeX = this.message.boundsDraw.width * scaleMod;

                    }

                    var messageSizeY = this.message.boundsDraw.height * scaleMod;

                    // crop text if overflowing

                    if ( messageSizeY > sizeY ) {

                        this.message.maxHeight = sizeY * scaleMod;

                        this.message.refresh( true );
                        messageSizeY = this.message.boundsDraw.height * scaleMod;
