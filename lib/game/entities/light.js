    ...
    ig.global.EntityLight = ig.EntityLight = ig.EntityExtended.extend({
        ...
        **/
		compute: function ( force ) {
			...
			if ( ( this.visible && ( this.performance === DYNAMIC && this.changed ) ) || force ) {
		...
		createLight: function () {
			...
			if ( this.pixelPerfect === true ) {
				...
				image.finalize( this.scaleComputedAt );
				
			}
			// when pixel perfect is not needed, it is much faster to use native methods
			else {
				
				var radiusScaled = this.radius * this.scaleComputedAt;
        ...
        cast: function () {
			...
			if ( this.pixelPerfect === true ) {
				...
				this.imageCast.finalize( this.scaleComputedAt );
		...
		recordChanges: function ( force ) {
			...
			if ( !_um.almostEqual( this.radiusDelta, 0, this.changeDeltaThreshold )
				|| !_um.almostEqual( this.alphaDelta, 0, this.changeDeltaThreshold ) ) {
			...
			if ( this.dynamicColor ) {
				...
				if ( ( !_um.almostEqual( this.rDelta, 0, this.changeDeltaThreshold )
					|| !_um.almostEqual( this.gDelta, 0, this.changeDeltaThreshold )
					|| !_um.almostEqual( this.bDelta, 0, this.changeDeltaThreshold ) ) ) {
