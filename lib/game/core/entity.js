...
ig.EntityExtended = ig.Entity.extend({
	...
	**/
	project: function ( light, context, origin, radius, a, b, originToA, originToB, aToB ) {
		...
		originToM.Normalize().Multiply( radius );
		originToA.Normalize().Multiply( radius );
		originToB.Normalize().Multiply( radius );
	...
	**/
	...
	recordChanges: function ( force ) {
		...
		if ( force
			|| !_um.almostEqual( this.posDelta.x, 0, this.changeDeltaThreshold )
			|| !_um.almostEqual( this.posDelta.y, 0, this.changeDeltaThreshold )
			|| !_um.almostEqual( this.angleDelta, 0, this.changeDeltaThreshold ) ) {
